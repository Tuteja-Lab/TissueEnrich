...
**Note:** The input gene set can either contain Ensembl Ids or Gene Symbols (specify this using `geneFormat`).
...
The `enrichmentOutput` is a list object containing the enrichment results. These results are explained in the next section.

### Exploring tissue-specific gene enrichment results

#### Tissue-specific gene enrichment histogram using `ggplot2`
The first object in the `enrichmentOutput` list is a dataframe containing the $-Log_{10}(P-Value)$, corresponding to the tissue-specific gene enrichment, along with the number of tissue-specific genes in the input gene set. This dataframe object can be used to visualize tissue-specific gene enrichment in the form of a histogram.

```{r eval=TRUE}
library(ggplot2)
ggplot(enrichmentOutput[[1]],aes(x=reorder(Tissue,-Log10PValue),y=Log10PValue,label = Tissue.Specific.Genes,fill = Tissue))+
...
The second object in the `enrichmentOutput` is a list containing the tissue-specific genes (with expression and tissue-specificity information) for genes from the input gene set. 

The first item in this list has expression values of the tissue-specific genes identified from the input gene set. The expression values can be visualized in the form of a heatmap. For example, the code below generates a heatmap showing the expression of the placenta specific genes across all the tissues.

```{r eval=TRUE}
library(tidyr)
library(ggplot2)
exp<-enrichmentOutput[[2]][["Placenta"]][[1]]
...
The second item in the second object of the `enrichmentOutput` list has tissue-specificity information for the input genes. The code below retrieves the tissue-specific genes along with the type of tissue-specificity in placenta tissue.

```{r eval=TRUE}
groupInf<-enrichmentOutput[[2]][["Placenta"]][[2]]
print(head(groupInf))
```


#### Retrieval of tissue-specific genes that could not be mapped
The third object in the `enrichmentOutput` list is a character vector that has a list of input genes that were not identified in the tissue-specific gene data.
```{r eval=TRUE}
print(enrichmentOutput[[3]])
```
...
The `teGeneRetrieval` function is used to define tissue-specific genes, using the algorithm from the HPA [@HPA2015]. It takes an SummarizedExperiment object containing expression information as input (rowData as genes and colData as tissue) and classifies the genes into different gene groups and stores into its metadata. The users also have the option of changing the default thresholds to vary the degree of tissue specificity of genes. More details about the gene groups and HPA thresholds are provided below.
...
As seen above, the metadata of the output consists of the tissue-specific genes information in a data frame with columns for Gene name, Tissue name, and Tissue-Specific group.
