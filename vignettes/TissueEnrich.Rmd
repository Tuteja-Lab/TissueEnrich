...
enrichmentOutput<-setNames(data.frame(assay(seEnrichmentOutput)),colData(seEnrichmentOutput)[,1])
...
```{r eval=TRUE}
library(tidyr)
seExp<-output[[2]][["Placenta"]][[1]]
exp<-setNames(data.frame(assay(seExp)),colData(seExp)[,1])
...
```{r eval=TRUE}
seGroupInf<-output[[2]][["Placenta"]][[2]]
