...
The second object in the `output` is a list containing the tissue-specific genes (with expression and tissue-specificity information) for genes from the input gene set. 
...
The second item in the second object of the `output` list has tissue-specificity information for the input genes. The code below retrieves the tissue-specific genes along with the type of tissue-specificity in placenta tissue.
...
The third object in the `output` list is a character vector that has a list of input genes that were not identified in the tissue-specific gene data.
```{r eval=TRUE}
print(output[[3]])
```
